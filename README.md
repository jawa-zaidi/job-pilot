...
>
> **First launch on macOS:** because the app isn't Apple-notarized yet, macOS may say
> *"JobPilot is damaged / can't be opened."* **Right-click the app → Open** (then confirm)
> the first time — or run `xattr -cr /Applications/JobPilot.app` in Terminal once. After
> that it opens normally. (Removing this prompt entirely needs the Apple Developer Program
> — $99/yr — for Developer ID signing + notarization.)
...
**Automated builds (GitHub Actions).** `.github/workflows/build.yml` builds **both** macOS (universal `.dmg`, Apple Silicon + Intel) and Windows (`.exe`):
- **Run manually** (Actions → *Build desktop installers* → Run workflow) to get both installers as downloadable artifacts.
- **Push a version tag** (`git tag v0.1.0 && git push --tags`) to build both and publish them to a **GitHub Release**, which also feeds the in-app auto-updater.

Builds are unsigned (see the install notes above for the one-time open steps).
